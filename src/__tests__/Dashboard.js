...
describe('Given I am connected as an Admin', () => {
  ...
  describe('When I am on Dashboard page, there are bills, and there is one pending', () => {  // filteredBills doit retourner 1 bill pending
    test('Then, filteredBills by pending status should return 1 bill', () => {
      const filtered_bills = filteredBills(bills, "pending")
      expect(filtered_bills.length).toBe(1)
    })
  })
  describe('When I am on Dashboard page, there are bills, and there is one accepted', () => {  // filteredBills doit retourner 1 bill accepted
    test('Then, filteredBills by accepted status should return 1 bill', () => {
      const filtered_bills = filteredBills(bills, "accepted")
      expect(filtered_bills.length).toBe(1)
    })
  })
  describe('When I am on Dashboard page, there are bills, and there is two refused', () => {  // filteredBills doit retourner 2 bills refused
    test('Then, filteredBills by accepted status should return 2 bills', () => {
      const filtered_bills = filteredBills(bills, "refused")
      expect(filtered_bills.length).toBe(2)
    })
  })
  describe('When I am on Dashboard page but it is loading', () => {               // quand le dashboard charge, la page doit afficher "loading"
    test('Then, Loading page should be rendered', () => {
      document.body.innerHTML = DashboardUI({ loading: true })
      expect(screen.getAllByText('Loading...')).toBeTruthy()
    })
  })
  describe('When I am on Dashboard page but back-end send an error message', () => {// quand une erreur est renvoyée, la page doit afficher "erreur"
    test('Then, Error page should be rendered', () => {
      document.body.innerHTML = DashboardUI({ error: 'some error message' })
      expect(screen.getAllByText('Erreur')).toBeTruthy()
    })
  })
  describe('When I am on Dashboard page and I click on arrow', () => {  // au clic sur un chevron, les bills doivent être affichées (handleShowTickets)
  ...
  describe('When I am on Dashboard page and I click on edit icon of a card', () => { // au clic sur une bill, le formulaire d'édition doit s'afficher
  ...
  describe('When I am on Dashboard page and I click 2 times on edit icon of a card', () => { // si 2 clics sur une bill, le filigrane doit s'afficher
  ...
  describe('When I am on Dashboard and there are no bills', () => {       // s'il n'y a pas de bill enregistrée, aucune bill ne doit être affichée (cards)
...
describe('Given I am connected as Admin, and I am on Dashboard page, and I clicked on a pending bill', () => {
  ...
  describe('When I click on accept button', () => {                     // au clic sur accepter, le filigrane doit s'afficher (pas le formulaire d'édition)
  ...
  describe('When I click on refuse button', () => {                     // au clic sur refuser, le filigrane doit s'afficher (pas le formulaire d'édition)
    test('I should be sent on Dashboard with big billed icon instead of form', () => {
      Object.defineProperty(window, 'localStorage', { value: localStorageMock })
      window.localStorage.setItem('user', JSON.stringify({
        type: 'Admin'
...
describe('Given I am connected as Admin and I am on Dashboard page and I clicked on a bill', () => {
  ...
  describe('When I click on the icon eye', () => {                                                     // au clic sur icon-eye, la modale doit s'afficher
...
describe("Given I am a user connected as Admin", () => {
  describe("When I navigate to Dashboard", () => {
    test("Then, bills are fetched from mock API GET", async () => {                             // les bills doivent être récupérées depuis les données mockées
    ...
    describe("When an error occurs on API", () => {              // QUAND ERREUR API
      ...
      test("fetches messages from an API and fails with 500 message error", async () => {                     // test erreur 500 lors du fetch
